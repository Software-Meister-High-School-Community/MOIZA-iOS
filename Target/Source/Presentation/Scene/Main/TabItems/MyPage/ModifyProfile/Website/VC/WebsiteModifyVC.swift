...
final class WebsiteModifyVC: BaseVC<WebsiteModifyReactor>{
    
    private let mainContainer = UIView()
    
    private let nextButton = NextButton(title: "확인")
    
    private let websiteTextfield = UITextField().then{
    ...
    override func configureNavigation() {
        self.navigationItem.setTitle(title: "웹사이트")
    }
    ...
    override func setLayout() {
        mainContainer.flex.marginHorizontal(16).define { flex in
            flex.addItem(websiteTextfield).marginTop(20).alignSelf(.center).width(100%).height(45)
            flex.addItem(nextButton).marginTop(20).width(50).height(28).alignSelf(.end)
        }
    }
    
    override func bindView(reactor: WebsiteModifyReactor) {
        nextButton.rx.tap
